(() => {
  ...
  async function init() {
    ...
    if (!/^\d{1,2}$/.test(chapterParam)) {
      showLoadError("The chapter number is not valid. Use a link such as ?chapter=2.", false);
      return;
    }
  ...
  function bindEvents() {
    ...
    document.querySelectorAll("button").forEach(button => {
      button.addEventListener("keydown", event => {
        if (["Enter", " ", "Space", "Spacebar"].includes(event.key) || event.code === "Space") {
  ...
  async function loadBank() {
    ...
    try {
      ...
      state.bank = bank;
      state.scaffold = state.scaffoldOverride || (["early", "mid", "late"].includes(bank.defaultScaffold) ? bank.defaultScaffold : "mid");
  ...
  function validateBank(bank) {
    ...
    for (const q of bank.questions) {
      const valid = q && typeof q.id === "string" && !ids.has(q.id) && q.chapter === state.chapter && categories.includes(q.category) &&
        Object.hasOwn(moveLabels, q.thinkingMove) && q.status === "approved" && q.practiceOnly === true &&
        typeof q.stem === "string" && q.stem.trim().length > 0 && Array.isArray(q.lectureIds) && q.lectureIds.length > 0 &&
        Array.isArray(q.choices) && q.choices.length === 4 &&
        q.choices.every(c => c && typeof c.id === "string" && typeof c.text === "string" && typeof c.feedback === "string") &&
        new Set(q.choices.map(c => c.id)).size === 4 && q.choices.some(c => c.id === q.correctChoiceId) &&
        typeof q.conciseCorrectFeedback === "string" && typeof q.conciseIncorrectFeedback === "string" &&
        typeof q.transferStrategy === "string" && q.sourceAlignment && typeof q.sourceAlignment.lectureClaim === "string" &&
        typeof q.accessibilityNotes === "string" && Array.isArray(q.walkthrough) && q.walkthrough.length >= 3 && q.walkthrough.length <= 5;
      if (valid) {
        ids.add(q.id);
        counts[q.category] += 1;
  ...
  function renderChoices(q) {
    ...
    q.choices.forEach((choice, index) => {
      ...
      input.addEventListener("keydown", event => {
        if ([" ", "Space", "Spacebar", "Enter"].includes(event.key) || event.code === "Space") {
          input.checked = true;
          event.preventDefault();
        }
      });
  ...
  function checkReasoning(event) {
    ...
    el["choice-feedback"].hidden = state.scaffold === "late";
  ...
  function resetFeedback() {
    ...
    el["choice-feedback"].hidden = false;
    el["walkthrough-region"].hidden = true;
    el["walkthrough-toggle"].setAttribute("aria-expanded", "false");
  }
  ...
  function toggleWalkthrough() {
    ...
    if (state.scaffold === "late") el["choice-feedback"].hidden = !open;
