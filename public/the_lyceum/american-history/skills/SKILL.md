...
- Exactly one `<aside class="notes">`, on the title slide. (This gate applies at deck-build time only. After the `study-buddy-notes` pass every slide carries one, which is expected — do not "fix" it back.)
