...
### When actual images exist (src path provided):
Use the `content-image-layout` with an `<img>` tag:
...
### When images need placeholders:
Use the `.img-placeholder` class with search terms:
...
8. **Create descriptive image alt text** with search terms when applicable
9. **Use pop-ups liberally** for terms, concepts, or additional context
10. **Keep slides uncluttered** — use pop-ups and notes for depth, not the slide face
